...
class VideoController extends Controller
{
    ...
    public function streamRanking()
    {
        ...
        foreach ($streamVideoList as $streamVideo) {
            if ($streamVideo->channelId === "0") { continue; }

            $videoIdList[] = $streamVideo->id;
            $channelIdList[] = $streamVideo->channelId;
            $streamVideoMap[$streamVideo->id] = [
                'channelId' => $streamVideo->channelId,
                'videoId' => $streamVideo->videoId,
                'videoName' => $streamVideo->videoName,
                'starttime' => $streamVideo->starttime,
            ];
        }
        $now = Carbon::now();

        // 15秒より短い場合は、取得が完了していない場合もあるため前の時間のものを取得する
        if ($now->second >= 15) {
    ...
    public function detail($videoId)
    {
        ...
        $concurrentViewersList = ConcurrentViewers::findAllByVideoId($video->id);
